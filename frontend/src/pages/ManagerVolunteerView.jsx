...
// Read-only week card — same look as the volunteer's own dashboard,
// but no add-task form since a manager is just viewing.
...
function ManagerVolunteerView() {
  ...
  useEffect(() => {
    ...
    async function loadProgress() {
      ...
      setFetchError("");
      try {
        const res = await api.get(`/api/manager/volunteer-progress/${volunteerId}`);
        if (cancelled) return;
        const data = res.data || {};
...
export default ManagerVolunteerView;